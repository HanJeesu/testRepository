...
public class LoginController extends HttpServlet {
	 ...
	 * -POST : url에 노출되지 않고 데이터 길이 제한이 없으며 즐겨찾기할 수 없다.
	 */
		
		//1)POST요청일 경우 전달값에 한글이 포합되어 있다면 encoding 설정을 해야 한다.
		request.setCharacterEncoding("UTF-8");
		
		//2)요청 시 전달한 데이터를 추출하기
		String userId = request.getParameter("userId");
		String userPwd = request.getParameter("userPwd");
		
		//조회해 온 Member객체(loginUser)
...
//		System.out.println("조회된 Member : "+m);
		...
		/*
		 * 응답 페이지에 전달할 값이 있다면 값을 어딘가엔 담아서 전달해야 한다.(이때 담아줄 수 있는 내장객체 4가지)
		 * -servlet scope
		 ...
		 * 3)request : request에 담은 데이터 해당 request를 포워딩한 응답jsp에서만 꺼내쓸 수 있다.
		...
		if(loginUser == null) {//로그인 실패(에러페이지로 응답해보기)
			...
			//session에 담으려면 Session객체를 먼저 가지고 와야한다.
...
//			RequestDispatcher view = request.getRequestDispatcher("응답 페이지 경로");
//			//포워딩(위임)-해당 경로로 view화면은 보여지지만 url은 변경되지 않는다.(맨 처음 요청했을 때의 url이 남아있음)
//			view.forward(request, response);
